...
class Solution(object):
    ...
    def dfs(self, maze, start, destination, visited):
        ...
        for i in range(4):
            res = self.helper(maze, start, dirs[i])
            if res == destination or self.dfs(maze, res, destination, visited):
                return True
        return False


    def helper(self, maze, start, dir):
        x, y = start[0]+dir[0], start[1] + dir[1]
        m, n = len(maze), len(maze[0])
        if x < 0 or x >= m or y < 0 or y >= n or maze[x][y] == 1:
             return start
        return self.helper(maze, [x, y], dir)
